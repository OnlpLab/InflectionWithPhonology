...
class MinimalTransducer(Transducer):
    ...
    def __init__(self, model, vocab, **kwargs):
        ...
        self.subst_i2w = {i : w[0] for w, i in self.vocab.act.w2i.items()
                          if i < self.NUM_ACTS and len(w) == 2 and w.endswith('@')}
        self.SUBSTS = list(self.subst_i2w.keys())
        print('{} SUBSTITUTION ACTIONS: '.format(len(self.SUBSTS)))
        print(', '.join(self.vocab.act.i2w[a] for a in self.SUBSTS))
        print('SUBST dictionary:')
        print(', '.join('({}={})'.format(k, v) for k, v in list(self.subst_i2w.items())))
...
#            return valid_actions
        ...
        if debug_mode:
            print()
            if oracle_actions: print(''.join([self.vocab.act.i2w[a] for a in oracle_actions]))
